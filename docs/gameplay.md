...
- 玩法描述：卫道士拥有专属默认怪物卡组，主题为高基础攻击和保守力量成长。默认牌组包含 3 张“斧劈”（1 费造成 6 点伤害）、2 张“重斧击”（2 费造成 10 点伤害）、1 张“行刑重击”（3 费造成 14 点伤害）、2 张“举斧戒备”（1 费获得 5 点格挡）和 2 张“狂信之力”（2 费获得 1 点力量并消耗）。因此卫道士整场战斗通过默认牌组最多获得 2 点永久力量，力量牌打出后会进入消耗堆，不会反复洗回牌组。卫道士使用斧类攻击牌时，会在攻击前和移动接近目标时举起斧头，只有收手下劈命中时才造成伤害并显示受击、格挡音效和伤害数字；玩家获得这些卡后使用时不会套用卫道士的举斧姿态，仍使用玩家自己的攻击表现。
