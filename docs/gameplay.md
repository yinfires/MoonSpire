...
- 玩法描述：怪物意图会预告怪物按当前手牌、费用、生命、格挡和状态计划使用的牌。怪物现在会按局势给可用牌评分，而不是只使用手牌中第一张符合类型的牌：攻击牌会优先选择能击倒玩家的目标，不能击倒时倾向于攻击发光、低生命或低格挡的玩家；防御牌会在怪物低生命、没有格挡或格挡较少时更容易被使用，已有较高格挡时降低优先级；治疗只在能实际恢复生命时计入收益；力量、守护、再生、迅捷、荆棘、引信等增益会倾向于给最需要的友方，已有同类状态较高时降低收益；流血、发光、扣除力量、中毒、烧伤、凋零、潮蚀、麻痹、饥饿、虚弱、缓慢等削弱会倾向于施加给最适合的玩家，已有同类状态较高时避免反复浪费。苦力怕只要手中有费用足够的 `点燃引信`，会优先打出它，再进入普通评分。消耗箭牌只有在怪物当前手牌中有可消耗的箭牌时才会作为有效攻击考虑。怪物一次行动后如果还剩费用和可用手牌，会继续按同一评分规则选择下一张牌；同分时仍保留手牌靠前者优先。僵尸、僵尸村民和僵尸猪灵默认使用 10 张牌的爪击/腐烂防御/猛扑/亡灵之力卡组；尸壳默认使用同源 10 张卡组，但把两张猛扑替换为饥饿猛扑；普通骷髅和凋灵骷髅默认使用 15 张牌的弓击/侧步/射击/箭卡组；沼骸默认使用 15 张牌的弓击/侧步/淬毒射击/箭卡组，其中淬毒射击按消耗箭规则消耗 1 张箭，基础造成 7 点远程攻击伤害，除被消耗箭自身附带的伤害和效果外，还额外给予目标 3 层中毒；流浪者默认使用 15 张牌的弓击/侧步/迟缓射击/箭卡组，其中迟缓射击按消耗箭规则消耗 1 张箭，基础造成 7 点远程攻击伤害，除被消耗箭自身附带的伤害和效果外，还额外给予目标 1 层缓慢；蜘蛛默认使用扑袭、躲闪、撕咬、蛛网卡组；洞穴蜘蛛默认使用扑袭、躲闪、蛛网、毒牙卡组；苦力怕默认使用 1 张点燃引信、3 张嘶嘶逼近、3 张火药外壳；守卫者默认使用 3 张守卫者光束、2 张潮蚀凝视、3 张棘刺甲壳、2 张深海回流；远古守卫者默认使用 3 张远古光束、2 张远古潮蚀、3 张远古棘冠、2 张深海压迫。守卫者与远古守卫者卡组以潮蚀和荆棘为核心；深海压迫为 2 费，给予目标 5 层潮蚀和 3 层缓慢，并消耗。开发者配置的怪物卡组可以是任意非空数量，不要求固定张数。
  - 代码实现：`BattleState.plannedMonsterCards()` 用虚拟手牌、虚拟费用和 `MonsterAiView` 反复调用 `chooseMonsterCard(...)` 生成意图列表，并模拟已计划牌造成的伤害、格挡、治疗和状态变化；`priorityMonsterCard(...)` 会在评分前优先选择可支付的 `builtin_monster_light_fuse`。`chooseMonsterCard(...)` 返回 `MonsterCardChoice`，包含手牌索引、单体目标和评分，评分 helper 会统一计算攻击、防御、增益、削弱、手牌处理和消耗箭收益，并把 `CardEffectKind.THORNS` 视为正面状态、把潮蚀、麻痹和饥饿视为负面状态计入评分与意图统计。`tickMonsterTurn()` 使用同一选择结果出牌并把评分选出的单体目标传入 `queueCard(...)`；多目标和随机目标仍由原有 `targetsForEffect(...)` 结算规则处理。`MoonSpireCardRegistry.builtinMonsterCards()` 注册蜘蛛系、苦力怕、溺尸、守卫者、远古守卫者以及淬毒射击/迟缓射击/饥饿猛扑内置牌；`BattleState` 将淬毒射击和迟缓射击的中毒/缓慢附加效果绑定到成功消耗箭后的远程结算，避免无箭时单独施加状态。`MonsterDeckProfile` 的默认卡组构建会先识别远古守卫者、守卫者、溺尸，尸壳，僵尸、僵尸村民、僵尸猪灵，流浪者、沼骸，骷髅、凋灵骷髅，蜘蛛和洞穴蜘蛛，以及苦力怕，再遍历对应列表生成实际战斗牌库；开发者覆盖卡组继续通过完整 `deckCardIds` 列表生成战斗牌库。
  - 变更记录：将怪物默认出牌从固定类型优先级改为局势评分式 AI，新增斩杀优先、发光/低血/低格挡目标偏好、低血防御偏好、增益/削弱避免浪费和消耗箭可用性判断；本次固化蜘蛛与洞穴蜘蛛默认卡组，新增蛛网、毒牙内置怪物牌，并按提供的开发者 JSON 将躲闪作为对应卡牌中文名；新增苦力怕原生牌组和点燃引信优先出牌规则；本次新增守卫者和远古守卫者默认卡组，并把远古守卫者伤害牌伤害与远古棘冠格挡按计划提升 2 点；本次将沼骸默认射击替换为淬毒射击，将流浪者默认射击替换为迟缓射击，并让尸壳默认卡组用饥饿猛扑替换原猛扑。
...
- 玩法描述：饥饿是负面战斗状态。拥有者回合开始时少抽 1 张牌，并减少 1 层；多层饥饿表示连续多个回合开始各少抽 1 张牌，不会在同一回合按层数一次少抽多张。固有牌仍会先进入手牌，饥饿只减少本次普通抽牌数。饥饿猛扑是尸壳使用的 2 费怪物牌，造成 8 点攻击伤害，并给予目标 1 层饥饿。
  - 代码实现：`BattleEffectType.HUNGER` 绑定 `gui/effects/hunger.png` 图标与翻译键；`BattleDeck.startTurn(RandomSource, int)` 在扣除固有牌后再应用抽牌减少量。`BattleState.beginPlayerTurn()` 在玩家抽牌前消耗玩家饥饿并传入抽牌减少量；玩家回合开始为敌方预抽意图手牌时只读取敌方当前饥饿计算少抽，不消耗层数；若敌方预抽后才首次获得饥饿，`BattleDeck.applyAdditionalStartTurnDrawReduction(...)` 会撤回一张本次回合开始抽到的牌；`beginMonsterTurn()` 在敌方回合开始时实际消耗敌方饥饿。`CardEffectKind.HUNGER` 接入卡牌结算、怪物 AI 模拟、意图评分、负面效果分类和卡牌描述/关键词提示；`MoonSpireCardRegistry` 注册 `builtin_monster_hungry_lunge`，`MonsterDeckProfile` 为尸壳在僵尸族分支前返回专属默认卡组。
  - 变更记录：新增饥饿状态、给予饥饿卡牌效果和饥饿猛扑；尸壳默认卡组从僵尸族拆出，只把原有两张猛扑替换为饥饿猛扑，普通僵尸、僵尸村民和僵尸猪灵仍使用原猛扑。
