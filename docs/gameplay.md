...
- 玩法描述：开发者可以为生物配置战斗开始时自带的战斗状态。被配置的生物一进入战斗就会带有对应层数的流血、守护、力量、再生、迅捷、中毒、烧伤、凋零、虚弱、缓慢、发光等状态；这些状态随后完全按现有状态规则结算，例如中毒会在回合开始时触发，烧伤和再生会在回合结束时触发，凋零会降低战斗内生命上限，发光会同步原版发光视觉。没有配置初始状态的生物仍保持原有开场状态。
...
- 玩法描述：怪物意图会预告怪物按当前手牌、费用、生命、格挡和状态计划使用的牌。怪物现在会按局势给可用牌评分，而不是只使用手牌中第一张符合类型的牌：攻击牌会优先选择能击倒玩家的目标，不能击倒时倾向于攻击发光、低生命或低格挡的玩家；防御牌会在怪物低生命、没有格挡或格挡较少时更容易被使用，已有较高格挡时降低优先级；治疗只在能实际恢复生命时计入收益；力量、守护、再生、迅捷、引信等增益会倾向于给最需要的友方，已有同类状态较高时降低收益；流血、发光、扣除力量、中毒、烧伤、凋零、虚弱、缓慢等削弱会倾向于施加给最适合的玩家，已有同类状态较高时避免反复浪费。苦力怕只要手中有费用足够的 `点燃引信`，会优先打出它，再进入普通评分。消耗箭牌只有在怪物当前手牌中有可消耗的箭牌时才会作为有效攻击考虑。怪物一次行动后如果还剩费用和可用手牌，会继续按同一评分规则选择下一张牌；同分时仍保留手牌靠前者优先。僵尸家族默认使用 10 张牌的爪击/腐烂防御/猛扑/亡灵之力卡组；骷髅家族默认使用 15 张牌的弓击/侧步/射击/箭卡组；蜘蛛默认使用扑袭、躲闪、撕咬、蛛网卡组；洞穴蜘蛛默认使用扑袭、躲闪、蛛网、毒牙卡组；苦力怕默认使用 1 张点燃引信、3 张嘶嘶逼近、3 张火药外壳。开发者配置的怪物卡组可以是任意非空数量，不要求固定张数。
