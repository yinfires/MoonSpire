...
- 玩法描述：岩浆怪默认使用近战火焰压迫风格怪物卡组，默认奖励池为这 5 张牌去重后的集合：`熔岩撞击` 1 费造成 4 点伤害；`灼热拍击` 1 费造成 3 点伤害并给予 1 层烧伤；`余烬黏附` 1 费给予 2 层烧伤；`火成身躯` 1 费获得 5 点格挡和 1 层荆棘；`喷发压迫` 2 费对全体敌方造成 3 点伤害并给予 1 层烧伤。默认牌组为熔岩撞击 3 张、灼热拍击 2 张、余烬黏附 2 张、火成身躯 2 张、喷发压迫 1 张。岩浆怪没有远程牌，也不会新增火球投射表现。自然进入战斗的大岩浆怪拥有 2 层分裂并使用完整默认卡组和大体型基准生命；中岩浆怪拥有 1 层分裂，卡组和生命按大体型基准削弱一次；小岩浆怪没有分裂，卡组和生命按大体型基准削弱两次。
  - 代码实现：`MoonSpireCardRegistry` 注册 `builtin_monster_magma_bump`、`builtin_monster_scorching_slap`、`builtin_monster_cinder_cling`、`builtin_monster_igneous_body` 和 `builtin_monster_eruptive_pressure`，这些牌均不带 `REMOTE` 效果；`MonsterDeckProfile` 为 `EntityType.MAGMA_CUBE` 返回 `MAGMA_CUBE_DEFAULT_DECK`，按 `Slime#getSize()` 计算削弱次数、默认生命和 `defaultSlimeSplitStacks(...)`，中/小体型动态牌使用 `dynamic_magma_cube_weakened_*` id，并让默认奖励池固定为完整岩浆怪牌组去重结果。`BattleState.applyDefaultInitialEffects(...)` 为自然岩浆怪添加对应分裂层数，现有 `Slime#setSize(...)` 子体缩小逻辑同时覆盖岩浆怪。
  - 变更记录：新增 5 张岩浆怪怪物牌、岩浆怪默认卡组、大小对应分裂层数和中/小岩浆怪削弱规则。
- 玩法描述：末影人默认使用“凝视追击 + 相位闪避”风格的 10 张怪物牌组，默认奖励池为 5 张专属牌去重后的集合：`末影凝视` 1 费给予单个敌人 2 层凝视；`闪烁步` 1 费获得 1 层相位和 1 层迅捷；`虚空爪击` 1 费造成 5 点伤害；`珍珠移形` 2 费获得 2 层相位和 4 点格挡；`撕裂凝视` 2 费造成 7 点伤害并给予 1 层虚弱。默认牌组为末影凝视 2 张、闪烁步 2 张、虚空爪击 3 张、珍珠移形 1 张、撕裂凝视 2 张。
  - 代码实现：`MoonSpireCardRegistry.builtinMonsterCards()` 注册 `builtin_monster_ender_stare`、`builtin_monster_blink_step`、`builtin_monster_void_claw`、`builtin_monster_pearl_shift` 和 `builtin_monster_rending_gaze`；`MonsterDeckProfile` 为 `EntityType.ENDERMAN` 返回 `ENDERMAN_DEFAULT_DECK`，奖励池沿用当前有效战斗牌组去重逻辑得到 5 张专属牌。
  - 变更记录：新增末影人 5 张内置怪物牌、10 张默认牌组和去重奖励池。
- 玩法描述：凋灵骷髅默认使用“近战压迫 + 凋零叠层”风格的 10 张怪物牌组，默认奖励池为 5 张专属牌去重后的集合：`凋零之刃` 1 费造成 5 点伤害并给予 1 层凋零；`焦骨防御` 1 费获得 5 点格挡；`灵魂劈斩` 2 费造成 9 点伤害并给予 2 层凋零；`黑堡架势` 2 费获得 7 点格挡和 1 层力量；`裂骨斩` 1 费造成 4 点伤害并给予 2 层流血。默认牌组为凋零之刃 3 张、焦骨防御 2 张、灵魂劈斩 2 张、黑堡架势 1 张、裂骨斩 2 张。凋灵骷髅不再继承普通骷髅的弓箭卡组，也不再默认获得箭矢充足状态。
  - 代码实现：`MoonSpireCardRegistry.builtinMonsterCards()` 注册 `builtin_monster_wither_blade`、`builtin_monster_charred_guard`、`builtin_monster_soul_cleave`、`builtin_monster_black_fortress_stance` 和 `builtin_monster_bone_rend`；`MonsterDeckProfile` 为 `EntityType.WITHER_SKELETON` 返回 `WITHER_SKELETON_DEFAULT_DECK`，并在通用骷髅家族分流前处理凋灵骷髅。`BattleState.visualStack(...)` 让裂骨斩使用石剑作为战斗手持物。`isSkeletonFamily(...)` 不再包含 `EntityType.WITHER_SKELETON`，因此 `hasAbundantArrowsByDefault(...)` 只继续作用于普通骷髅、流浪者、沼骸和掠夺者。
  - 变更记录：新增凋灵骷髅 5 张内置怪物牌、10 张默认牌组和去重奖励池，并移除其默认弓箭卡组与箭矢充足继承。
- 玩法描述：普通猪灵默认使用“弩矢压制 + 镀金近战”混合风格的 15 张怪物牌组，默认奖励池为当前牌组去重后的集合：`猪灵弩矢` 1 费，远程，消耗 1 张箭造成 7 点伤害；`镀金斩击` 1 费造成 5 点伤害并给予 1 层流血；`黄金戒备` 1 费获得 5 点格挡；`箭` 作为弩矢消耗资源。默认牌组为猪灵弩矢 4 张、镀金斩击 3 张、黄金戒备 3 张、箭 5 张，普通猪灵默认拥有 1 层备箭充足。猪灵蛮兵默认使用“高伤精英近战”风格的 10 张怪物牌组，默认奖励池为 5 张专属牌去重后的集合：`蛮兵重斩` 1 费造成 8 点伤害；`蛮兵劈裂` 2 费造成 12 点伤害并给予 2 层流血；`蛮兵压迫` 2 费造成 9 点伤害并给予 1 层虚弱；`蛮兵金甲` 1 费获得 7 点格挡；`蛮兵怒火` 2 费获得 1 层力量并消耗。默认牌组为蛮兵重斩 3 张、蛮兵劈裂 2 张、蛮兵压迫 2 张、蛮兵金甲 2 张、蛮兵怒火 1 张；猪灵蛮兵不使用弩牌、箭牌，也不默认拥有备箭充足。
  - 代码实现：`MoonSpireCardRegistry.builtinMonsterCards()` 注册 `builtin_monster_piglin_bolt`、`builtin_monster_gilded_cut`、`builtin_monster_gold_guard`、`builtin_monster_brute_chop`、`builtin_monster_brute_cleave`、`builtin_monster_brute_pressure`、`builtin_monster_brute_gold_plate` 和 `builtin_monster_brute_fury`；`MonsterDeckProfile` 为 `EntityType.PIGLIN` 返回 `PIGLIN_DEFAULT_DECK`，为 `EntityType.PIGLIN_BRUTE` 返回 `PIGLIN_BRUTE_DEFAULT_DECK`。`MonsterDeckProfile.hasAbundantArrowsByDefault(...)` 只把普通猪灵加入默认备箭充足名单，奖励池沿用当前有效战斗牌组去重逻辑。
  - 变更记录：新增普通猪灵 3 张内置怪物牌、猪灵蛮兵 5 张内置怪物牌、两套默认牌组和去重奖励池；普通猪灵获得弩与近战混合路线，猪灵蛮兵获得不依赖箭的高伤精英近战路线。
- 玩法描述：僵尸猪灵默认使用“复仇金剑”风格的 10 张怪物牌组，默认奖励池为 5 张专属牌去重后的集合：`复仇金斩` 1 费造成 5 点伤害并给予 1 层流血；`腐金戒备` 1 费获得 5 点格挡；`僵化猛扑` 2 费造成 8 点伤害并给予 1 层凋零；`咒金架势` 2 费获得 6 点格挡和 1 层力量并消耗；`不息复仇` 1 费造成 4 点伤害并给予 1 层凋零。默认牌组为复仇金斩 3 张、腐金戒备 2 张、僵化猛扑 2 张、咒金架势 1 张、不息复仇 2 张。僵尸猪灵不使用普通僵尸卡组，也不使用猪灵弩牌、箭牌或默认备箭充足。
  - 代码实现：`MoonSpireCardRegistry.builtinMonsterCards()` 注册 `builtin_monster_vengeful_gold_cut`、`builtin_monster_rotten_gold_guard`、`builtin_monster_zombified_lunge`、`builtin_monster_cursed_gold_stance` 和 `builtin_monster_restless_revenge`；`MonsterDeckProfile` 为 `EntityType.ZOMBIFIED_PIGLIN` 返回 `ZOMBIFIED_PIGLIN_DEFAULT_DECK`，并在通用僵尸家族分流前处理僵尸猪灵，`isZombieFamily(...)` 不再包含 `EntityType.ZOMBIFIED_PIGLIN`。`BattleState.visualStack(...)` 通过猪灵手持物映射让攻击牌显示金剑、防御和架势牌显示金胸甲。
  - 变更记录：新增僵尸猪灵 5 张内置怪物牌、10 张默认牌组和去重奖励池，并从普通僵尸默认卡组中分离僵尸猪灵。
- 玩法描述：疣猪兽默认使用“獠牙冲撞 + 厚皮蓄势”风格的 10 张怪物牌组，默认奖励池为 5 张专属牌去重后的集合：`疣猪獠牙` 1 费造成 6 点伤害；`绯红头槌` 1 费造成 5 点伤害并给予 1 层缓慢；`獠牙上挑` 2 费造成 9 点伤害并给予 1 层虚弱；`绯红厚皮` 1 费获得 7 点格挡；`群兽怒火` 2 费获得 1 层力量和 1 层迅捷并消耗。默认牌组为疣猪獠牙 3 张、绯红头槌 2 张、獠牙上挑 2 张、绯红厚皮 2 张、群兽怒火 1 张。僵尸疣猪兽默认使用“腐烂冲撞 + 死皮压制”风格的 10 张怪物牌组，默认奖励池为 5 张专属牌去重后的集合：`僵尸獠牙` 1 费造成 7 点伤害；`腐烂头槌` 1 费造成 6 点伤害并给予 1 层凋零；`狂乱冲撞` 2 费造成 10 点伤害并给予 1 层虚弱；`死皮防御` 1 费获得 6 点格挡和 1 层守护；`腐烂践踏` 2 费对全体敌方造成 4 点伤害并给予 1 层缓慢。默认牌组为僵尸獠牙 3 张、腐烂头槌 2 张、狂乱冲撞 2 张、死皮防御 2 张、腐烂践踏 1 张。疣猪兽或僵尸疣猪兽打出专属攻击牌时，会播放兼容原版模型的抬头攻击表现；玩家获得并打出这些牌时使用通用近战表现，不强制套用怪物专用头部动画。
  - 代码实现：`MoonSpireCardRegistry.builtinMonsterCards()` 注册 `builtin_monster_hoglin_gore`、`builtin_monster_crimson_headbutt`、`builtin_monster_tusks_up`、`builtin_monster_crimson_hide`、`builtin_monster_herd_fury`、`builtin_monster_zoglin_gore`、`builtin_monster_rotten_headbutt`、`builtin_monster_maddened_charge`、`builtin_monster_dead_hide` 和 `builtin_monster_rotting_trample`；`MonsterDeckProfile` 为 `EntityType.HOGLIN` 返回 `HOGLIN_DEFAULT_DECK`，为 `EntityType.ZOGLIN` 返回 `ZOGLIN_DEFAULT_DECK`，奖励池沿用当前有效战斗牌组去重逻辑。`BattleVisualEvent.AnimationType.HOGLIN_HEAD_ATTACK`、`BattleState.LungeStyle.HOGLIN_HEAD_ATTACK`、`ClientBattleState.visualHoglinAttackTick(...)` 和 `ClientEvents.syncVisualHoglinHeadAttacks(...)` 共同驱动 `Hoglin.attackAnimationRemainingTicks` 与 `Zoglin.attackAnimationRemainingTicks`，并在动画结束或战斗状态清理时归零。
  - 变更记录：新增疣猪兽与僵尸疣猪兽各 5 张内置怪物牌、两套默认牌组、去重奖励池和兼容原版抬头攻击的世界动画通路。
...
- 玩法描述：旋风人默认使用风弹扰乱风格怪物卡组，默认奖励池为这 5 张牌去重后的集合：`风弹` 1 费远程造成 5 点伤害；`疾风爆发` 1 费远程造成 3 点伤害并给予 1 层缓慢；`横扫劲风` 2 费远程对全体敌方造成 3 点伤害；`旋流护身` 1 费获得 5 点格挡；`乱流压迫` 1 费给予 2 层虚弱。默认牌组为风弹 3 张、疾风爆发 2 张、横扫劲风 1 张、旋流护身 2 张、乱流压迫 2 张。旋风人打出风弹类远程牌时会播放原版风弹发射与命中爆发风格的世界表现；玩家获得并打出这些牌，或把原版风弹物品转换为`风弹`牌后打出时，也会使用兼容玩家的风弹投射、投掷音效和命中爆发表现。
  - 代码实现：`MoonSpireCardRegistry` 注册 `builtin_monster_wind_charge`、`builtin_monster_gale_burst`、`builtin_monster_sweeping_gust`、`builtin_monster_whirling_guard` 和 `builtin_monster_unsteady_air`，并把 `minecraft:wind_charge` 物品转换为带有 `REMOTE` 与 `DAMAGE` 的 `item_minecraft_wind_charge` 牌；`MonsterDeckProfile` 为 `EntityType.BREEZE` 返回 `BREEZE_DEFAULT_DECK`。`BattleVisualEvent.AnimationType.WIND_CHARGE`、`BattleState.isWindChargeCard(...)`、`ClientBattleState.ProjectileVisual`、`BattleWorldOverlay.RenderOnlyProjectileEntities` 和 `ClientEvents` 共同把风弹类远程牌表现为原版风弹视觉实体、投掷/发射音效、命中爆发音效与阵风粒子，实际伤害和状态仍由服务端卡牌效果结算。
  - 变更记录：新增 5 张旋风人怪物牌、旋风人默认卡组、风弹专用战斗动画类型，以及旋风人和玩家都可使用的风弹投射表现；本次补充原版风弹物品转换牌的风弹动画识别，并让风弹视觉投射物到达目标时播放风弹爆发音效和阵风粒子。
...
- 玩法描述：普通骷髅、流浪者、沼骸、掠夺者和普通猪灵默认在战斗开始时拥有 1 层“备箭充足”。备箭充足会在回合开始时，每层生成 1 张转换卡“箭”；如果手牌已经达到 10 张上限，生成的箭会改为进入弃牌堆。凋灵骷髅使用专属近战卡组，猪灵蛮兵使用专属高伤近战卡组，二者都不默认拥有备箭充足。
  - 代码实现：`MonsterDeckProfile.hasAbundantArrowsByDefault(...)` 识别普通骷髅、流浪者、沼骸、掠夺者和普通猪灵；`BattleState.applyDefaultInitialEffects(...)` 为这些敌方初始添加 `BattleEffectType.ABUNDANT_ARROWS`，`applyOwnTurnStartEffects(...)` 通过 `MoonSpireCardRegistry.cardInstance("item_minecraft_arrow")` 生成箭，`BattleDeck.addGeneratedToHandOrDiscard(...)` 负责手牌满时放入弃牌堆。
  - 变更记录：新增“备箭充足”战斗状态，并让骷髅家族默认拥有该状态；之后让掠夺者默认也拥有该状态，保证其弩牌按消耗箭规则运作；本次将凋灵骷髅从备箭充足默认名单中移除，并让普通猪灵默认拥有该状态以支持弩牌，猪灵蛮兵不加入默认名单；中文状态说明默认写“回合开始时/回合结束时”，不额外写“自己的”，规则语义仍按行动方自己的回合结算。
...
- 玩法描述：玩家方胜利且战斗完全结束后，每名参战玩家会按战斗开始时的敌方怪物顺序依次获得卡牌奖励机会。每个怪物只弹出一次奖励页；奖励页从该怪物完整且去重的奖励池中随机抽取最多 3 张不重复卡牌。点击卡牌会把该卡加入玩家卡组并进入下一个怪物奖励页；点击“跳过”不会获得卡牌，也会进入下一页。空奖励池不会弹页；玩家方失败、取消战斗或没有胜利结算时不会发放奖励。
  - 代码实现：`BattleState` 在战斗初始化时记录战斗开始敌方顺序和每个敌方的 `MonsterRewardPool`，后续结算只读取这批开始时敌方。`MonsterDeckProfile.rewardPoolCardIds(...)` 读取完整去重奖励池，默认由该怪物当前战斗卡组去重得到，若开发者配置了奖励池覆盖则读取覆盖列表。`BattleManager.endBattle(...)` 只在 `playerVictory()` 为真时为每名参战玩家创建奖励队列，每页随机洗牌后截取最多 3 张 `CardInstance`，通过 `OpenCardRewardScreenPayload` 打开客户端奖励界面；客户端选择或跳过后发送 `SelectCardRewardPayload`，服务端校验奖励 id、页序号和候选卡 UUID 后才把选择的卡加入 `PlayerCardData` 并同步。
  - 变更记录：新增战斗胜利后的逐怪物卡牌奖励流程；奖励抽取基于完整去重奖池，不会因界面只显示 3 张而丢失池内其它候选。
...
- 玩法描述：玩家和非玩家生物拥有速度属性。玩家基础速度为 5，玩家以外的生物基础速度以僵尸为基准整体换算：僵尸基础速度为 5，其它怪物以及后续可能参战的友方生物都按自身移动速度相对僵尸移动速度缩放；洞穴蜘蛛固定基础速度为 9。每个战斗回合开始时双方基础速度随机加减 2 得到本回合速度。速度只在目标没有格挡时影响攻击牌造成的直接伤害：攻击方速度更高时增伤，防御方速度更高时减伤；目标的格挡会吸收已经完成速度和守护修正后的直接伤害。敌方回合中，高本回合速度的敌方优先行动；本回合速度相同时，按敌方条目从上到下的顺序行动。
  - 代码实现：`CombatantState` 保存基础速度和本回合速度，`MonsterDeckProfile.defaultBaseSpeed(...)` 使用 `CardBalance.NON_PLAYER_BASELINE_MOVEMENT_SPEED` 作为玩家以外生物的速度换算基准，并对 `EntityType.CAVE_SPIDER` 返回 9；`BattleState.beginRound()` 投掷本回合速度，`CombatantState.applyCardDamage()` 调用 `BattleDamageCalculator.directDamage()` 按目标当前格挡决定是否套用速度乘区，再用目标格挡吸收伤害，剩余部分扣除生命；`BattleState.beginMonsterTurn()` 构建按 `CombatantState.roundSpeed()` 降序排列的 `enemyTurnOrder`，同速时保留 `enemyStates` 原始条目顺序。
  - 变更记录：速度从卡牌移到生物战斗状态中；伤害预览和实际结算统一为先计算总伤害，再由格挡吸收；玩家以外生物的基础速度改为以僵尸为基准整体换算，僵尸基础速度与玩家一致为 5；本次将洞穴蜘蛛默认基础速度固定为 9；速度增减伤改为只在目标无格挡时触发；新增敌方按本回合速度决定行动先后，同速按条目从上到下行动。
...
- 玩法描述：怪物意图会预告怪物按当前手牌、费用、生命、格挡和状态计划使用的牌。怪物现在会按局势给可用牌评分，而不是只使用手牌中第一张符合类型的牌：攻击牌会优先选择能击倒玩家的目标，不能击倒时倾向于攻击发光、低生命或低格挡的玩家；防御牌会在怪物低生命、没有格挡或格挡较少时更容易被使用，已有较高格挡时降低优先级；治疗只在能实际恢复生命时计入收益；力量、守护、再生、迅捷、荆棘、引信等增益会倾向于给最需要的友方，已有同类状态较高时降低收益；流血、发光、扣除力量、中毒、烧伤、凋零、潮蚀、麻痹、饥饿、虚弱、缓慢等削弱会倾向于施加给最适合的玩家，已有同类状态较高时避免反复浪费。苦力怕只要手中有费用足够且有效的点燃引信，就会作为特殊生物规则优先使用，再进入普通分类加权随机。消耗箭牌只有在怪物当前手牌中有可消耗的箭牌时才会作为有效攻击考虑。怪物一次行动后如果还剩费用和可用手牌，会继续按同一评分规则选择下一张牌；同分时仍保留手牌靠前者优先。僵尸、僵尸村民和僵尸猪灵默认使用 10 张牌的爪击/腐烂防御/猛扑/亡灵之力卡组；尸壳默认使用同源 10 张卡组，但把两张猛扑替换为饥饿猛扑；普通骷髅和凋灵骷髅默认使用 15 张牌的弓击/侧步/射击/箭卡组；沼骸默认使用 15 张牌的弓击/侧步/淬毒射击/箭卡组，其中淬毒射击按消耗箭规则消耗 1 张箭，基础造成 7 点远程攻击伤害，除被消耗箭自身附带的伤害和效果外，还额外给予目标 3 层中毒；流浪者默认使用 15 张牌的弓击/侧步/迟缓射击/箭卡组，其中迟缓射击按消耗箭规则消耗 1 张箭，基础造成 7 点远程攻击伤害，除被消耗箭自身附带的伤害和效果外，还额外给予目标 1 层缓慢；蜘蛛默认使用扑袭、躲闪、撕咬、蛛网卡组；洞穴蜘蛛默认使用扑袭、躲闪、蛛网、毒牙卡组；苦力怕默认使用 1 张点燃引信、3 张嘶嘶逼近、3 张火药外壳；守卫者默认使用 3 张守卫者光束、2 张潮蚀凝视、3 张棘刺甲壳、2 张深海回流；远古守卫者默认使用 3 张远古光束、2 张远古潮蚀、3 张远古棘冠、2 张深海压迫。守卫者与远古守卫者卡组以潮蚀和荆棘为核心；深海压迫为 2 费，给予目标 5 层潮蚀和 3 层缓慢，并消耗。开发者配置的怪物卡组可以是任意非空数量，不要求固定张数。
  - 代码实现：`BattleState.plannedMonsterCards()` 用虚拟手牌、虚拟费用和 `MonsterAiView` 反复调用 `chooseMonsterCard(...)` 生成意图列表，并模拟已计划牌造成的伤害、格挡、治疗和状态变化；玩家回合展示意图时 `MonsterTurnPlan.cards(...)` 使用计划中记录的费用顺序解析卡牌 UUID，敌方回合实际出牌时 `MonsterTurnPlan.pollUsable(...)` 使用当前手牌索引、当前费用和当前目标重新校验。`chooseMonsterCard(...)` 返回 `MonsterCardChoice`，包含卡牌 UUID、手牌索引、单体目标、类别和评分；`priorityMonsterCard(...)` 会在苦力怕拥有可用 `builtin_monster_light_fuse` 时先返回点燃引信，其它情况再由分类加权随机处理。评分 helper 会统一计算攻击、防御、增益、削弱、手牌处理和消耗箭收益，并把 `CardEffectKind.THORNS` 视为正面状态、把潮蚀、麻痹和饥饿视为负面状态计入评分与意图统计。`tickMonsterTurn()` 消费同一计划出牌并把评分选出的单体目标传入 `queueCard(...)`；多目标和随机目标仍由原有 `targetsForEffect(...)` 结算规则处理。`MoonSpireCardRegistry.builtinMonsterCards()` 注册蜘蛛系、苦力怕、溺尸、守卫者、远古守卫者以及淬毒射击/迟缓射击/饥饿猛扑内置牌；`BattleState` 将淬毒射击和迟缓射击的中毒/缓慢附加效果绑定到成功消耗箭后的远程结算，避免无箭时单独施加状态。`MonsterDeckProfile` 的默认卡组构建会先识别远古守卫者、守卫者、溺尸，尸壳，僵尸、僵尸村民、僵尸猪灵，流浪者、沼骸，骷髅、凋灵骷髅，蜘蛛和洞穴蜘蛛，以及苦力怕，再遍历对应列表生成实际战斗牌库；开发者覆盖卡组继续通过完整 `deckCardIds` 列表生成战斗牌库。
  - 变更记录：将怪物默认出牌从固定类型优先级改为局势评分式 AI，新增斩杀优先、发光/低血/低格挡目标偏好、低血防御偏好、增益/削弱避免浪费和消耗箭可用性判断；本次固化蜘蛛与洞穴蜘蛛默认卡组，新增蛛网、毒牙内置怪物牌，并按提供的开发者 JSON 将躲闪作为对应卡牌中文名；新增苦力怕原生牌组，并保留点燃引信作为苦力怕特殊优先出牌规则；本次新增守卫者和远古守卫者默认卡组，并把远古守卫者伤害牌伤害与远古棘冠格挡按计划提升 2 点；本次将沼骸默认射击替换为淬毒射击，将流浪者默认射击替换为迟缓射击，并让尸壳默认卡组用饥饿猛扑替换原猛扑。
- 玩法描述：掠夺者默认使用 15 张牌的扣下悬刀/掠过割伤/装填掩护/箭卡组。扣下悬刀为核心弩牌，2 费，按消耗箭规则消耗 1 张箭，基础造成 10 点远程攻击伤害；掠过割伤为 1 费近战低伤牌，造成 3 点伤害并给予目标 2 层流血；装填掩护为 1 费，获得 4 点格挡。
  - 代码实现：`MoonSpireCardRegistry.builtinMonsterCards()` 注册 `builtin_monster_drop_the_hanging_blade`、`builtin_monster_grazing_cut` 和 `builtin_monster_reload_cover`，其中扣下悬刀和装填掩护使用 `minecraft:crossbow` 作为卡图/表现物品；`MonsterDeckProfile` 在识别 `EntityType.PILLAGER` 时返回专属 15 张默认牌组；扣下悬刀通过既有 `CONSUME_ARROW` 远程链路在成功消耗箭后结算伤害，掠过割伤不带 `REMOTE`，因此沿用普通近战冲刺和命中结算。
...
- 玩法描述：蠹虫默认使用 10 张“群聚增殖”牌组：3 张“啮咬”（1 费，造成 3 点伤害）、2 张“裂石颚击”（1 费，造成 2 点伤害并给予 1 层缓慢）、2 张“石缝疾行”（1 费，获得 3 点格挡和 1 层迅捷）、2 张“群响警觉”（1 费，所有友方获得 2 点格挡，自身获得 1 层迅捷）和 1 张“虫蚀呼唤”（2 费，召唤 1 只存活 2 回合的蠹虫，消耗）。正常蠹虫和召唤出的蠹虫使用完全相同的默认卡组，因此召唤出的蠹虫后续也能抽到并打出“虫蚀呼唤”，继续召唤新的蠹虫；蠹虫召唤时会优先落在召唤者左右侧的地面安全位置，不会生成在身体中心或前方默认点，也会避开其它参战单位。没有额外硬性召唤上限，扩张由费用、抽牌、消耗、召唤物存活回合和战斗结束规则自然限制。玩家通过奖励或指令获得“虫蚀呼唤”后打出时，召唤出的蠹虫加入玩家方，并按玩家方友军流程自动行动。
  - 代码实现：`MoonSpireCardRegistry` 注册 `builtin_monster_nipping_bite`、`builtin_monster_crackling_mandibles`、`builtin_monster_stone_scuttle`、`builtin_monster_swarm_alarm` 和 `builtin_monster_infested_call`；`MonsterDeckProfile` 为 `EntityType.SILVERFISH` 返回包含重复牌的专属默认卡组，召唤物创建战斗牌堆时同样调用 `MonsterDeckProfile.createDeck(...)`，因此会读取与正常参战蠹虫一致的默认卡组或开发者覆盖。`builtin_monster_infested_call` 使用通用 `CardEffectKind.SUMMON` 并把 `entityTypeId` 写为 `minecraft:silverfish`；`BattleState.summonEntities(...)` 解析实体 id、创建 `EntityType.SILVERFISH`、加入施放者同侧、应用召唤物存活层数、注册动态参战单位并同步快照，`safeSummonGroundPosition(...)` 使用蠹虫实际碰撞盒搜索脚下有碰撞且自身不进方块的位置，`stabilizeSummonedEntity(...)`、召唤物锁位逻辑和客户端 `SUMMONED` 待机稳定逻辑会清空移动状态，并保持蠹虫在选中的落点上。
  - 变更记录：新增蠹虫专属默认卡组、5 张内置蠹虫怪物牌和“虫蚀呼唤”召唤牌；召唤出的蠹虫不使用精简牌组，也不阻止继续召唤蠹虫；本次调整蠹虫召唤落点，防止卡在地里或与其它单位重叠；本次补充服务端与客户端的待机稳定规则，减少刚召唤后到首次行动前的抽搐。
...
- 玩法描述：幻翼拥有专属默认怪物卡组，主题为俯冲骚扰、攻击降速和格挡提速。默认牌组包含 3 张“掠空俯冲”（1 费造成 5 点伤害并给予目标 1 层缓慢）、2 张“拖拽利爪”（2 费造成 7 点伤害并给予目标 1 层缓慢）、3 张“振翼防御”（1 费获得 3 点格挡并给予自身 1 层迅捷）和 2 张“月影滑翔”（2 费获得 5 点格挡并给予自身 1 层迅捷）。因此幻翼会通过攻击压低目标速度，并通过防御牌提升自己的速度；速度变化继续使用迅捷、缓慢以及现有速度差伤害修正规则。
  - 代码实现：`MoonSpireCardRegistry` 注册 4 张 `builtin_monster_*` 幻翼牌，复用 `DAMAGE`、`SLOWNESS`、`BLOCK` 和 `HASTE` 卡牌效果，不新增状态或 UI 资源；`MonsterDeckProfile` 为 `EntityType.PHANTOM` 返回专属 10 张默认卡组。卡牌描述继续由现有描述构建器根据数值和效果自动生成，`CardInstance` 保留名称键到内置 card id 的归一化映射。
  - 变更记录：新增幻翼专属默认卡组和 4 张内置怪物牌，使幻翼不再使用通用怪物 fallback 卡组。

- 玩法描述：恼鬼拥有专属默认怪物卡组，主题为高速突刺、短暂游移和轻量防御。默认牌组包含 3 张“锐刃突袭”（1 费造成 5 点伤害）、2 张“闪影切割”（1 费造成 4 点伤害并给予自身 1 层迅捷）、2 张“穿影刺击”（1 费造成 4 点伤害并给予目标 1 层缓慢）、2 张“闪避游移”（1 费获得 4 点格挡并给予自身 1 层迅捷）和 1 张“狂乱俯冲”（2 费造成 8 点伤害并给予 2 层流血）。恼鬼使用这些攻击牌时，会在攻击前进入原版红色裂纹形态并抬手，保持蓄势姿态接近目标；只有到达命中帧后才造成伤害、显示受击、格挡音效和伤害数字。“闪避游移”是防御牌，不触发攻击蓄势表现；玩家获得这些牌后使用时不套用恼鬼的红色裂纹姿态，仍使用玩家自己的攻击表现。
  - 代码实现：`MoonSpireCardRegistry` 注册 5 张恼鬼专属怪物牌并使用 `minecraft:iron_sword` 作为卡图/视觉持物，`MonsterDeckProfile` 为 `EntityType.VEX` 返回专属 10 张默认卡组。`BattleState` 对恼鬼 4 张攻击牌使用 `BattleVisualEvent.AnimationType.VEX_CHARGE_LUNGE`，通过 `LungeStyle.VEX_CHARGE` 先发出无伤害的蓄势接近视觉事件，再在命中帧结算卡牌效果；`ClientBattleState` 按客户端 tick 推进 8 tick 原地蓄势、7 tick 蓄势接近和命中后的恢复阶段，`ClientEvents` 只在该视觉期间临时调用原版 `Vex#setIsCharging(true)`，并在动画结束后恢复原始 charging 状态。
  - 变更记录：新增恼鬼高速突刺默认卡组和 5 张内置怪物牌；新增恼鬼专用蓄势突刺世界表现，保证红色裂纹抬手发生在攻击前，伤害发生在命中帧。

- 玩法描述：卫道士拥有专属默认怪物卡组，主题为高基础攻击和保守力量成长。默认牌组包含 3 张“斧劈”（1 费造成 6 点伤害并给予 2 层流血）、2 张“重斧击”（2 费造成 10 点伤害）、1 张“行刑重击”（3 费造成 14 点伤害并给予 5 层流血）、2 张“举斧戒备”（1 费获得 5 点格挡）和 2 张“狂信之力”（2 费获得 1 点力量并消耗）。因此卫道士整场战斗通过默认牌组最多获得 2 点永久力量，力量牌打出后会进入消耗堆，不会反复洗回牌组。卫道士使用斧类攻击牌时，会在攻击前和移动接近目标时举起斧头，只有收手下劈命中时才造成伤害并显示受击、格挡音效和伤害数字；玩家获得这些卡后使用时不会套用卫道士的举斧姿态，仍使用玩家自己的攻击表现。
  - 代码实现：`MoonSpireCardRegistry` 注册卫道士专属怪物牌并使用 `minecraft:iron_axe` 作为卡图/视觉持物，`MonsterDeckProfile` 为 `EntityType.VINDICATOR` 返回专属 10 张默认牌组。`BattleState` 对卫道士三张斧类攻击牌使用 `BattleVisualEvent.AnimationType.VINDICATOR_AXE_SWING`，先发出无伤害的举斧移动视觉事件，再在专用命中帧结算卡牌效果；`ClientBattleState` 按客户端 tick 推进举斧前摇、举斧接近和收手恢复，`ClientEvents` 只在卫道士该视觉期间临时设置原版 aggressive 姿态并在命中事件触发主手挥击。
  - 变更记录：新增卫道士默认高攻力量牌组，力量获取改为少量带消耗的永久增益；新增卫道士专用斧击世界表现，保证伤害发生在收手命中时而不是举斧或接近开始时。
...
- 玩法描述：开发者可以为非玩家生物配置覆盖血量、费用、速度和卡组；战斗开始时对应生物会使用覆盖值。未配置的字段继续使用原有生物血量、由移动速度换算出的速度和固定费用。敌对怪物在没有卡组覆盖时使用对应默认怪物牌，其中僵尸家族默认 10 张、骷髅家族默认 15 张，其它怪物使用各自默认卡组；中立和友善生物没有默认卡组，必须由开发者添加至少 1 张卡才可交战。开发者显式清空某个生物卡组时，该空卡组也会作为覆盖保存，使该生物无法交战。
...
- 玩法描述：饥饿是负面战斗状态。拥有者回合开始时少抽 1 张牌，并减少 1 层；多层饥饿表示连续多个回合开始各少抽 1 张牌，不会在同一回合按层数一次少抽多张。固有牌仍会先进入手牌，饥饿只减少本次普通抽牌数。饥饿猛扑是尸壳使用的 2 费怪物牌，造成 8 点攻击伤害，并给予目标 1 层饥饿。
  ...
  - 变更记录：新增饥饿状态、给予饥饿卡牌效果和饥饿猛扑；尸壳默认卡组从僵尸族拆出，只把原有两张猛扑替换为饥饿猛扑，普通僵尸、僵尸村民和僵尸猪灵仍使用原猛扑；本次修复敌方预抽后获得饥饿时怪物意图不会随少抽牌结果立即更新的问题。
...
- 玩法描述：删除全局自定义卡后，未来战斗、怪物自定义卡组和在线玩家收藏/牌组不会继续保留这张已经无法解析的卡；删除或缺失开发者数据文件后重新打开开发者中心，也不会把旧内存缓存里的自定义内容重新写回磁盘。怪物默认卡组允许重复牌，旧 id 或无效开发者覆盖被清空后，僵尸、骷髅等有默认卡组的怪物会回退到代码内置默认卡组。
  - 代码实现：`DeveloperDataManager.reload()` 在打开开发者中心时按磁盘当前状态重读数据，`sanitizeData(...)` 对卡牌、卡面和怪物覆盖去重，清理怪物卡组中无法解析的卡牌 id，但保留怪物卡组顺序和重复数量；如果非空覆盖清理后变成空卡组且对应实体有默认卡组，则取消无效卡组覆盖。`MonsterDeckProfile.createDeck(...)` 和 `hasBattleDeck(...)` 在运行时也会对无法解析的非空覆盖回退默认卡组。开发者中心删除卡牌时同步移除怪物卡组引用和显示名缓存。`SaveDeveloperDataPayload` 保存后遍历在线玩家的 `PlayerCardData.removeUnresolvableCustomCards()`，移除已删除或无法解析的自定义卡并通过附件同步和 `BattleManager.syncCardData(...)` 刷新客户端。
  - 变更记录：修复旧自定义卡、旧 id 卡或已删除文件内容在退出重进后从缓存、怪物卡组或在线玩家收藏中复活，以及坏的开发者怪物卡组覆盖导致僵尸、骷髅默认卡组丢失的问题。
...
- 玩法描述：溺尸不再继承僵尸族默认牌组，默认使用 10 张海洋敌对生物牌：3 张“三叉戟投掷”、1 张“引雷投掷”、2 张“激流冲刺”和 4 张“鹦鹉螺壳”。“三叉戟投掷”费用 1，远程造成 5 点攻击伤害并给予 1 层潮蚀；“引雷投掷”费用 2，远程造成 6 点攻击伤害并给予 1 层潮蚀和 1 层麻痹；“激流冲刺”费用 2，造成 8 点攻击伤害并给予 2 层潮蚀；“鹦鹉螺壳”费用 1，获得 6 点格挡。潮蚀会在获得格挡时先抵消格挡，鹦鹉螺壳实际获得的格挡也会被目标身上的潮蚀正确削减。
  - 代码实现：`MonsterDeckProfile` 为 `EntityType.DROWNED` 单独返回溺尸默认牌组，并在僵尸族判定前分流；`MoonSpireCardRegistry` 注册 4 张 `builtin_monster_*` 溺尸牌，三叉戟牌使用 `minecraft:trident` 作为卡图和临时手持视觉物品，鹦鹉螺壳使用 `minecraft:nautilus_shell`。`CardEffectKind.TIDAL_EROSION` 和 `CardEffectKind.PARALYSIS` 接入卡牌描述、怪物 AI 评分、目标预览、战斗结算和保存/加载映射。
  - 变更记录：新增溺尸专属默认牌组和 4 张内置牌；新增卡牌可施加潮蚀与麻痹效果，玩家和怪物使用同一套卡牌效果结算规则。
...
- 玩法描述：烈焰人不再使用通用怪物回退牌组，默认使用 10 张火焰专属牌：3 张“烈焰火球”、2 张“烈焰连射”、2 张“余焰护身”、2 张“热浪摇曳”和 1 张“烈焰压迫”。“烈焰火球”费用 1，远程造成 5 点伤害并给予 1 层烧伤；“烈焰连射”费用 2，远程造成 3 点伤害 2 次并给予 1 层烧伤；“余焰护身”费用 1，获得 5 点格挡和 1 层荆棘；“热浪摇曳”费用 1，获得 3 点格挡和 1 层迅捷；“烈焰压迫”费用 2，远程对所有敌方造成 3 点伤害并给予 1 层烧伤。
  - 代码实现：`MoonSpireCardRegistry` 注册 5 张 `builtin_monster_blaze_*` 内置怪物牌，远程火球牌带 `CardEffectKind.REMOTE`，数值和烧伤仍由普通卡牌效果结算；`MonsterDeckProfile` 为 `EntityType.BLAZE` 返回 `BLAZE_DEFAULT_DECK`，奖励池默认从开战牌组去重生成。
  - 变更记录：新增烈焰人专属默认牌组和 5 张内置牌；卡面图案暂时留空，只使用卡牌名称、费用、数值和效果文本展示规则。
- 玩法描述：恶魂不再使用通用怪物回退牌组，默认使用 10 张下界远程牌：3 张“恶魂火球”、2 张“爆裂哀嚎”、2 张“硫磺漂移”、2 张“泪光护幕”和 1 张“地狱尖啸”。“恶魂火球”费用 1，远程造成 6 点伤害并给予 1 层烧伤；“爆裂哀嚎”费用 2，远程对所有敌方造成 4 点伤害并给予 1 层虚弱；“硫磺漂移”费用 1，获得 4 点格挡和 1 层相位；“泪光护幕”费用 1，获得 6 点格挡和 1 层守护；“地狱尖啸”费用 1，对所有敌方给予 1 层虚弱。
  - 代码实现：`MoonSpireCardRegistry` 注册 5 张 `builtin_monster_ghast_*` 内置怪物牌；`MonsterDeckProfile` 为 `EntityType.GHAST` 返回 `GHAST_DEFAULT_DECK`。`CardInstance.normalizeCardId(...)` 将新增 nameKey 映射回稳定 card id，兼容只保存 nameKey 的旧卡牌实例。
  - 变更记录：新增恶魂专属默认牌组和 5 张内置牌；玩家之后获得这些牌时使用同一套伤害、格挡和状态效果结算规则。
