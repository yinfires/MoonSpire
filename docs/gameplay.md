...
- 玩法描述：开发者可以为生物配置战斗开始时自带的战斗状态。被配置的生物一进入战斗就会带有对应层数的流血、守护、力量、再生、迅捷、中毒、烧伤、虚弱、缓慢、发光等状态；这些状态随后完全按现有状态规则结算，例如中毒会在拥有者自己的回合开始时触发，烧伤和再生会在拥有者自己的回合结束时触发，发光会同步原版发光视觉。没有配置初始状态的生物仍保持原有开场状态。
...
- 玩法描述：怪物意图会预告怪物按当前手牌、费用、生命、格挡和状态计划使用的牌。怪物现在会按局势给可用牌评分，而不是只使用手牌中第一张符合类型的牌：攻击牌会优先选择能击倒玩家的目标，不能击倒时倾向于攻击发光、低生命或低格挡的玩家；防御牌会在怪物低生命、没有格挡或格挡较少时更容易被使用，已有较高格挡时降低优先级；治疗只在能实际恢复生命时计入收益；力量、守护、再生、迅捷等增益会倾向于给最需要的友方，已有同类状态较高时降低收益；流血、发光、扣除力量、中毒、烧伤、虚弱、缓慢等削弱会倾向于施加给最适合的玩家，已有同类状态较高时避免反复浪费。消耗箭牌只有在怪物当前手牌中有可消耗的箭牌时才会作为有效攻击考虑。怪物一次行动后如果还剩费用和可用手牌，会继续按同一评分规则选择下一张牌；同分时仍保留手牌靠前者优先。默认怪物卡组仍保持 5 张，开发者配置的怪物卡组可以是任意非空数量，不要求固定张数。
  - 代码实现：`BattleState.plannedMonsterCards()` 用虚拟手牌、虚拟费用和 `MonsterAiView` 反复调用 `chooseMonsterCard(...)` 生成意图列表，并模拟已计划牌造成的伤害、格挡、治疗和状态变化；`chooseMonsterCard(...)` 返回 `MonsterCardChoice`，包含手牌索引、单体目标和评分，评分 helper 会统一计算攻击、防御、增益、削弱、手牌处理和消耗箭收益。`tickMonsterTurn()` 使用同一选择结果出牌并把评分选出的单体目标传入 `queueCard(...)`；多目标和随机目标仍由原有 `targetsForEffect(...)` 结算规则处理。`MonsterDeckProfile` 的默认卡组内容仍是 5 张，但默认卡组构建改为遍历列表生成，不再依赖固定五参数方法；开发者覆盖卡组继续通过完整 `deckCardIds` 列表生成战斗牌库。
  - 变更记录：将怪物默认出牌从固定类型优先级改为局势评分式 AI，新增斩杀优先、发光/低血/低格挡目标偏好、低血防御偏好、增益/削弱避免浪费和消耗箭可用性判断；解除怪物默认卡组构建代码对固定五张参数的依赖，同时保持现有默认卡组数量不变。
