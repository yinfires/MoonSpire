...
- 玩法描述：开发者可以为生物配置战斗开始时自带的战斗状态。被配置的生物一进入战斗就会带有对应层数的流血、守护、力量、再生、迅捷、中毒、烧伤、虚弱、缓慢、发光等状态；这些状态随后完全按现有状态规则结算，例如中毒会在回合开始时触发，烧伤和再生会在回合结束时触发，发光会同步原版发光视觉。没有配置初始状态的生物仍保持原有开场状态。
...
- 玩法描述：战斗中双方不能自主移动、原版攻击或原版交互；进入战斗时会清除进入前保留的水平移动、跳跃输入和按键输入，怪物会转向玩家。玩家在半空进入战斗时，战斗的回退点会记录为正下方可站立地面位置，而不是半空坐标；战斗中玩家仍可自然下落。卡牌世界动画可以驱动非自主移动：近战直接伤害会让攻击者短暂冲到目标前并停留在打击点，距离足够近时不移动，玩家近战冲刺会显示平滑位移和行走腿部摆动；带有“远程”的直接伤害会使用弓或弩使用姿态并射出可穿过方块的箭，没有表现物品的远程牌默认使用弓表现。该规则同时适用于怪物牌和玩家牌，例如猛扑会冲到目标前再攻击，射击会调用远程弓攻击动画。动画移动无视方块阻挡，但不会恢复玩家输入或怪物 AI 自主移动；双方回合都结束、动画取消、逃离或战斗结束回位时会优先回到无碰撞安全落点，避免实体被锁进方块。卡牌造成血量伤害时，受击者仍会像原版受击一样被轻微击退并向上击飞，击退结束后在新的落点再次锁定，且双方回合都结束时双方回到战斗开始位置。
  - 代码实现：`ServerBattleEvents` 取消战斗中的原版攻击、右键、丢弃和容器行为，`BattleState` 通过碰撞体查找玩家脚下可站立位置作为回退点，并在开战、回合结束与战斗结束时传送回去；`lockBattleEntities()` 每 tick 清除非受击窗口内的水平速度、跳跃状态和移动偏移，但保留 Y 轴速度与重力；`beginBattleAnimation(...)` 对非远程直接伤害创建 `LungeAnimation`，对远程直接伤害创建 `ProjectileAnimation`，空表现物品时补上弓作为视觉物品；当 `BattleState` 存在服务器权威 `BattleAnimation` 时，动画拥有的实体可用 `noPhysics` 与 `MoverType.SELF` 或玩家服务端权威位置更新进行非自主移动，箭实体使用 `setNoPhysics(true)` 和 `setNoGravity(true)` 被引导到目标中心，因此箭和冲击都不会被方块阻挡；`ClientBattleState.VisualState` 为玩家近战视觉保存冲刺起点和目标点，`ClientEvents` 在客户端 tick 推进行走动画，并在渲染前对玩家实体叠加平滑视觉偏移；`safeReturnPosition(...)`、`restoreSurvivor(...)` 和 `EntityLock` 在重置/结束/取消时恢复原 `noPhysics` 状态并向上扫描无碰撞落点；`applyBattleKnockback()` 在卡牌造成血量伤害时按原版方向调用 `LivingEntity.knockback(...)`，并放开冻结直到受击者落地或释放窗口结束；`ClientEvents` 在客户端同步清除移动键和非受击窗口内的本地水平预测速度。
  - 变更记录：战斗从实体追逐和自动攻击改为纯卡牌结算；补强进入战斗时的残余水平移动清理、半空自然下落、怪物朝向玩家、服务端同步的受击击退/击飞表现，以及卡牌动画驱动的无碰撞非自主移动；修正半空开战后的回退点和受击击退方向、落地锁定时机；本次新增玩家近战冲刺的客户端平滑视觉偏移和腿部摆动，近距离不移动，近战命中后不立即后退，并让双方回合结束、动画取消、逃离和战斗结束恢复都使用安全无碰撞落点。
...
- 玩法描述：内置怪物卡牌使用英文翻译派生的 canonical id：`builtin_monster_claw`、`builtin_monster_undead_power`、`builtin_monster_lunge`、`builtin_monster_shoot` 和 `builtin_monster_bow_strike`。拼音 id 不再作为有效内置卡保留；后续新增同显示名卡牌时必须加入稳定区分项生成唯一 id，不能复用已有 id。
  ...
  - 变更记录：将爪击、亡灵之力、猛扑、射击、弓击改为英文 canonical id，并移除拼音怪物牌 id 兼容路径。
...
- 玩法描述：怪物意图会预告怪物按当前手牌、费用、生命、格挡和状态计划使用的牌。怪物现在会按局势给可用牌评分，而不是只使用手牌中第一张符合类型的牌：攻击牌会优先选择能击倒玩家的目标，不能击倒时倾向于攻击发光、低生命或低格挡的玩家；防御牌会在怪物低生命、没有格挡或格挡较少时更容易被使用，已有较高格挡时降低优先级；治疗只在能实际恢复生命时计入收益；力量、守护、再生、迅捷等增益会倾向于给最需要的友方，已有同类状态较高时降低收益；流血、发光、扣除力量、中毒、烧伤、虚弱、缓慢等削弱会倾向于施加给最适合的玩家，已有同类状态较高时避免反复浪费。消耗箭牌只有在怪物当前手牌中有可消耗的箭牌时才会作为有效攻击考虑。怪物一次行动后如果还剩费用和可用手牌，会继续按同一评分规则选择下一张牌；同分时仍保留手牌靠前者优先。僵尸家族默认使用 10 张牌的爪击/腐烂防御/猛扑/亡灵之力卡组；骷髅家族默认使用 15 张牌的弓击/侧步/射击/箭卡组；其它默认怪物仍使用各自默认卡组。开发者配置的怪物卡组可以是任意非空数量，不要求固定张数。
  - 代码实现：`BattleState.plannedMonsterCards()` 用虚拟手牌、虚拟费用和 `MonsterAiView` 反复调用 `chooseMonsterCard(...)` 生成意图列表，并模拟已计划牌造成的伤害、格挡、治疗和状态变化；`chooseMonsterCard(...)` 返回 `MonsterCardChoice`，包含手牌索引、单体目标和评分，评分 helper 会统一计算攻击、防御、增益、削弱、手牌处理和消耗箭收益。`tickMonsterTurn()` 使用同一选择结果出牌并把评分选出的单体目标传入 `queueCard(...)`；多目标和随机目标仍由原有 `targetsForEffect(...)` 结算规则处理。`MonsterDeckProfile` 的默认卡组构建会先识别僵尸、尸壳、溺尸、僵尸村民、僵尸猪灵，以及骷髅、流浪者、沼骸、凋灵骷髅，再遍历对应列表生成实际战斗牌库；开发者覆盖卡组继续通过完整 `deckCardIds` 列表生成战斗牌库。
  - 变更记录：将怪物默认出牌从固定类型优先级改为局势评分式 AI，新增斩杀优先、发光/低血/低格挡目标偏好、低血防御偏好、增益/削弱避免浪费和消耗箭可用性判断；本次将僵尸家族默认卡组改为 10 张、骷髅家族默认卡组改为 15 张，并让同家族怪物共享对应默认卡组。
...
- 玩法描述：再生、迅捷、中毒、烧伤、虚弱、缓慢、备箭充足都是战斗状态。再生在回合结束时恢复等同于层数的生命值，然后减少 1 层；迅捷让拥有者战斗内有效速度提高等同于层数的点数；中毒在回合开始时造成等同于层数的伤害，然后减少 1 层；烧伤在回合结束时造成等同于层数的伤害，然后减少 1 层；虚弱让拥有者造成的攻击伤害降低至 75%，并在回合结束时减少 1 层；缓慢让拥有者战斗内有效速度降低等同于层数的点数；备箭充足在回合开始时每层获得一张转换卡箭。迅捷与缓慢只影响本场战斗里的有效速度，有效速度最低为 1。
  - 代码实现：`BattleEffectType` 新增这些状态；`CombatantState.roundSpeed()` 返回基础本回合速度加迅捷、减缓慢后的有效速度并限制最低 1；`BattleState.beginPlayerTurn()`、`beginMonsterTurn()` 和 `beginRoundEnd()` 分别在行动方自己的回合开始/结束触发中毒、备箭充足、再生、烧伤和虚弱衰减；状态伤害沿用 `applyEffectDamage()`，会先被格挡吸收但不受虚弱、守护和速度影响。
  - 变更记录：补齐战斗状态触发时机、层数衰减、速度修正和状态伤害规则；本次新增备箭充足的回合开始生成箭规则。
...
- 玩法描述：单位因假死亡导致手牌离开时，不会把保留牌或其它离手牌显示成中央打出卡牌；只有本地实际提交出的牌和服务端下发的出牌视觉事件会触发打出展示。战斗结束或回合重置把怪物放回开战位置时，会优先寻找不碰撞的安全落点，避免卡进地下。
  - 代码实现：`BattleScreen.syncSnapshotAnimations(...)` 只把 `locallyUsedCardIds` 中的离手牌视为本地出牌移除，不再因为 `resolvingEffects` 把所有离手牌都判定为打出；`BattleState.resetParticipantsToStart()` 使用 `safeReturnPosition(...)` 从起点向上扫描无碰撞位置后再传送并锁定实体。
