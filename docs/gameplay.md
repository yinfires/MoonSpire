...
- 玩法描述：怪物意图会预告怪物按当前手牌、费用、生命、格挡和状态计划使用的牌。怪物现在会按局势给可用牌评分，而不是只使用手牌中第一张符合类型的牌：攻击牌会优先选择能击倒玩家的目标，不能击倒时倾向于攻击发光、低生命或低格挡的玩家；防御牌会在怪物低生命、没有格挡或格挡较少时更容易被使用，已有较高格挡时降低优先级；治疗只在能实际恢复生命时计入收益；力量、守护、再生、迅捷等增益会倾向于给最需要的友方，已有同类状态较高时降低收益；流血、发光、扣除力量、中毒、烧伤、虚弱、缓慢等削弱会倾向于施加给最适合的玩家，已有同类状态较高时避免反复浪费。消耗箭牌只有在怪物当前手牌中有可消耗的箭牌时才会作为有效攻击考虑。怪物一次行动后如果还剩费用和可用手牌，会继续按同一评分规则选择下一张牌；同分时仍保留手牌靠前者优先。僵尸家族默认使用 10 张牌的爪击/腐烂防御/猛扑/亡灵之力卡组；骷髅家族默认使用 15 张牌的弓击/侧步/射击/箭卡组；蜘蛛默认使用扑袭、躲闪、撕咬、蛛网卡组；洞穴蜘蛛默认使用扑袭、躲闪、蛛网、毒牙卡组。开发者配置的怪物卡组可以是任意非空数量，不要求固定张数。
  - 代码实现：`BattleState.plannedMonsterCards()` 用虚拟手牌、虚拟费用和 `MonsterAiView` 反复调用 `chooseMonsterCard(...)` 生成意图列表，并模拟已计划牌造成的伤害、格挡、治疗和状态变化；`chooseMonsterCard(...)` 返回 `MonsterCardChoice`，包含手牌索引、单体目标和评分，评分 helper 会统一计算攻击、防御、增益、削弱、手牌处理和消耗箭收益。`tickMonsterTurn()` 使用同一选择结果出牌并把评分选出的单体目标传入 `queueCard(...)`；多目标和随机目标仍由原有 `targetsForEffect(...)` 结算规则处理。`MoonSpireCardRegistry.builtinMonsterCards()` 注册蜘蛛系内置牌，`MonsterDeckProfile` 的默认卡组构建会先识别僵尸、尸壳、溺尸、僵尸村民、僵尸猪灵，骷髅、流浪者、沼骸、凋灵骷髅，以及蜘蛛和洞穴蜘蛛，再遍历对应列表生成实际战斗牌库；开发者覆盖卡组继续通过完整 `deckCardIds` 列表生成战斗牌库。
  - 变更记录：将怪物默认出牌从固定类型优先级改为局势评分式 AI，新增斩杀优先、发光/低血/低格挡目标偏好、低血防御偏好、增益/削弱避免浪费和消耗箭可用性判断；本次固化蜘蛛与洞穴蜘蛛默认卡组，新增蛛网、毒牙内置怪物牌，并按提供的开发者 JSON 将躲闪作为对应卡牌中文名。
