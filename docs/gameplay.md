...
- 玩法描述：开发者可以为怪物配置覆盖血量、费用、速度和卡组；战斗开始时对应怪物会使用覆盖值。未配置的字段继续使用原有怪物血量、由移动速度换算出的速度和固定费用。默认怪物牌库为 5 张牌，开发者覆盖卡组也按实际配置的卡牌列表使用，不再自动扩充。
  - 代码实现：`DeveloperMonsterDefinition` 保存怪物覆盖数据；`BattleState` 在创建怪物 `CombatantState` 时优先读取覆盖血量、费用和速度；`MonsterDeckProfile` 在有覆盖卡组时使用全局卡牌生成怪物牌库，未覆盖时按实体类型生成 5 张默认怪物牌。
  - 变更记录：新增开发者怪物覆盖配置，并将怪物默认牌库和覆盖牌库统一为 5 张实际卡牌列表。
- 玩法描述：开发者中心的怪物卡组改为专用列表界面编辑。开发者在怪物页点击“打开卡组”后，可以在怪物当前卡组中单选一张卡并进行添加、删除、复制、重置或完成；添加时进入全卡牌列表，可搜索并同时选择多张卡，确认后追加到该怪物卡组。未保存覆盖的怪物会在编辑器中显示默认怪物血量、费用、速度和 5 张默认卡组作为基础；已有覆盖只替换对应字段。怪物页还可以将当前怪物覆盖重置到上一次保存状态，或经确认删除该怪物覆盖，使该怪物回到默认战斗数据。
  - 代码实现：`DeveloperCenterScreen` 负责怪物页属性表单、打开怪物卡组界面、重置和删除确认；未覆盖字段通过临时创建对应实体读取默认最大生命和移动速度，并复用 `CardBalance.fixedEnergy()` 与战斗速度换算公式。保存回包后会保留当前选中的怪物 id 和怪物列表滚动位置；`DeveloperMonsterDeckScreen` 维护当前怪物卡组列表和全卡牌多选添加列表，并通过 `DeveloperMonsterDefinition.deckCardIds()` 写回覆盖卡组；`MonsterDeckProfile.defaultDeckCardIds(...)` 提供与战斗默认卡组一致的编辑器回落数据。
  - 变更记录：移除怪物页手填卡牌 id 的编辑方式，新增怪物卡组编辑 UI、全卡牌搜索添加 UI、默认基础数值和 5 张默认卡组读取、保存后保留怪物选中与列表滚动状态，以及怪物覆盖重置/删除操作。
