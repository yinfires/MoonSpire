...
- 玩法描述：`凝视` 是负面战斗状态。目标每次受到任意卡牌直接攻击时，本段攻击额外受到等同凝视层数的伤害；同一张卡有多段直接攻击时，每段都会触发凝视加伤，凝视只在这张卡打向该目标的最后一段直接攻击结算后清除。`相位` 是正面战斗状态。目标每次受到卡牌直接攻击时消耗 1 层，使本段攻击最终伤害降低 50%，向上取整且至少降低 1 点；相位不影响流血、荆棘、燃烧、中毒等效果伤害。流血和荆棘也以“每段卡牌直接攻击”为触发单位，因此单卡多段攻击会逐段触发。
  - 代码实现：`CardEffectKind` 和 `BattleEffectType` 增加 `GAZE`、`PHASE`；`BattleState.applyPendingCardBatch(...)` 在统一直接攻击入口逐段处理凝视加伤、相位消耗减伤、流血自伤和荆棘反伤，并通过剩余 `PendingCardBatch` / `PendingCardStep` 检查把凝视清除延后到同一卡最后一段直接攻击之后。`CombatantState.applyCardDamage(...)` 接收凝视额外伤害并只对卡牌直接伤害应用相位减伤；`BattleDamageCalculator.phaseReducedDamage(...)` 统一计算相位减伤；`CardRenderHelper`、`BattleScreen`、`BattleWorldOverlay` 和怪物 AI 预览/评分同步识别这两个状态。
  - 变更记录：新增凝视和相位状态，并把流血、荆棘、凝视、相位的直接攻击触发粒度统一调整为单卡多段攻击的每一段。
...
- 玩法描述：骷髅家族和掠夺者默认在战斗开始时拥有 1 层“备箭充足”。备箭充足会在回合开始时，每层生成 1 张转换卡“箭”；如果手牌已经达到 10 张上限，生成的箭会改为进入弃牌堆。
  - 代码实现：`MonsterDeckProfile.hasAbundantArrowsByDefault(...)` 识别骷髅、流浪者、沼骸、凋灵骷髅和掠夺者；`BattleState.applyDefaultInitialEffects(...)` 为这些敌方初始添加 `BattleEffectType.ABUNDANT_ARROWS`，`applyOwnTurnStartEffects(...)` 通过 `MoonSpireCardRegistry.cardInstance("item_minecraft_arrow")` 生成箭，`BattleDeck.addGeneratedToHandOrDiscard(...)` 负责手牌满时放入弃牌堆。
  - 变更记录：新增“备箭充足”战斗状态，并让骷髅家族默认拥有该状态；本次让掠夺者默认也拥有该状态，保证其弩牌按消耗箭规则运作；中文状态说明默认写“回合开始时/回合结束时”，不额外写“自己的”，规则语义仍按行动方自己的回合结算。
