...
- 玩法描述：`黑暗` 是负面战斗状态。带黑暗的单位打出攻击牌时，费用会正常消耗，卡牌会从手牌移除并直接进入弃牌堆，黑暗减少 1 层，且不会结算任何卡牌效果；回合结束时黑暗也减少 1 层。被黑暗拦截的攻击牌即使带有消耗，也仍按黑暗规则进入弃牌堆。固定伤害是攻击牌伤害类型，会直接扣除目标战斗生命，不受速度、力量、虚弱、守护、发光、凝视、相位、格挡等加减伤或防护影响。
  - 代码实现：`BattleEffectType.DARKNESS` 定义状态与 `effects/darkness.png` 图标，`CardEffectKind.DARKNESS` 负责施加状态，`CombatantState.decayEndOfTurnEffects()` 处理回合结束衰减。`BattleState.usePlayerCard(...)`、自动友方/敌方出牌和 `discardCardForDarkness(...)` 在目标校验、费用消耗和移出手牌后拦截攻击牌并丢弃；队列和 `applyPendingCardBatch(...)` 因提前返回不会触发伤害、状态、抽牌、箭矢、手牌选择、流血、荆棘、凝视/相位或世界动画。`CardEffectKind.FIXED_DAMAGE` 通过 `CombatantState.applyDirectHealthDamage(...)` 直接扣血，预览、怪物 AI、意图和卡面数值也使用固定伤害路径。
  - 变更记录：新增黑暗状态、黑暗出牌拦截、固定伤害效果和对应预览/意图/AI 识别。
...
- 玩法描述：敌方回合中，怪物自动选择手牌时不再固定打出最高分的攻击牌或防御牌，而是先在状态、功能、攻击、防御等可用类别之间按权重随机，再在选中的类别内按评分随机；苦力怕的点燃引信仍作为特殊生物规则，在当前可用时优先使用。负面状态牌仍优先作用于玩家，增益状态牌仍优先作用于自己或其它敌方单位；目标已拥有较多同类状态时，效果牌评分会降低。怪物意图会提前锁定计划，玩家回合显示意图时按怪物下一次行动可用的计划费用预览，不受上一轮怪物剩余费用影响；进入敌方回合后会消费同一套锁定计划，意图区只显示剩余计划牌，不会在行动延迟或效果结算期间重抽另一套随机意图。计划会预演抽牌与获得能量，因此抽牌、回费后解锁的后续牌也会参与意图；实际出牌前仍会重新检查当前手牌、费用和目标。如果计划中的牌费用变得不够、已离开手牌或目标失效，会在执行瞬间跳过该项并用当前手牌重新补选，因此抽牌、生成牌、弃牌、消耗、保留降费、获得能量和后续费用变化都能正确影响怪物后续行动。
  - 代码实现：`BattleState` 为每只敌方和自动友军缓存 `MonsterTurnPlan`，计划项保存卡牌 UUID、预览牌、目标、类别、评分、计划抽到的卡牌 UUID 和计划获得的能量；`plannedAutoChoices(...)` 使用虚拟牌库预演手牌、抽牌堆、弃牌堆、手牌上限和剩余费用，`DRAW_CARDS` 会把虚拟抽到的牌加入计划，`GAIN_ENERGY` 会增加虚拟剩余费用。`MonsterTurnPlan.cards(...)` 展示意图时按计划费用或当前费用顺序解析 UUID，未真实入手的计划抽牌使用预览牌显示；`MonsterTurnPlan.pollUsable(...)` 在实际行动前按 UUID 重新定位当前手牌并校验当前 `card.cost()`、当前能量、目标和可用性，失效后通过同一随机选择逻辑即时补选。真实结算 `DRAW_CARDS` 时优先按计划 UUID 抽同一批牌；若牌库状态变化导致计划抽牌无法兑现，则回退随机抽牌并让后续计划重建。进入敌方回合时不会刷新已有计划，只为缺失计划的敌方补建计划，避免行动开始时重抽另一套意图。
  - 变更记录：怪物 AI 从最高分贪心改为分类加权随机，并保留苦力怕点燃引信优先规则；意图计划兼容手牌增减、费用变化和目标失效，减少怪物长期执着于攻击牌或防御牌的问题；本次修复怪物在下一玩家回合因上一轮剩余费用为 0 而不显示意图，并稳定敌方回合开始时的预览与实际出牌计划；本次让怪物和自动友军的抽牌、回费真实改变后续自动行动，并提前纳入意图预览。
- 玩法描述：挑战目标范围改为 10 格。玩家指向 10 格内可交战生物时，准星旁会提示按挑战目标键发起挑战；该提示遵循原版 HUD 隐藏状态，按 F1 隐藏 HUD 时同步隐藏。被指向的目标必定加入敌方，不要求它已经敌视玩家。敌对怪物默认拥有怪物卡组；中立和友善生物默认没有卡组，因此默认不能被挑战。开发者为任意可自定义生物配置非空卡组后，该生物才会成为可挑战目标；如果某个生物的最终卡组内容为空，即使它有其它属性覆盖，也不能交战。战斗开始时，10 格内有有效卡组且未参战的其它玩家加入玩家方；10 格内对任一参战玩家有敌意、有非空战斗卡组且未参战的生物加入敌方。
  - 代码实现：`BattleHud` 和 `ClientEvents.CHALLENGE` 共同使用 `ClientEvents.challengeTarget()` 的 10 格客户端目标检测；本次改为优先读取 Minecraft 当前原版命中的 `LivingEntity`，只有原版命中为空时才回退到自定义 AABB 射线扫描。`BattleHud.render()` 在 `minecraft.options.hideGui` 为真时跳过准星旁提示绘制；命中后发送 `ChallengeTargetPayload`。客户端提示和服务端 `BattleManager.canChallenge()` 都通过 `MonsterDeckProfile.hasBattleDeck(...)` 判断目标是否有最终可用卡组；`BattleScreen.worldTargetUnderMouse(...)` 也会先尝试复用当前原版命中的战斗单位，再回退到战斗相机射线，因此闭壳潜影贝等碰撞盒特殊的生物更容易被正确选中。`collectPlayers()` 和 `collectEnemies()` 收集参战玩家、被挑战目标和附近敌意且有卡组的生物，并把所有参战实体登记到同一个 `BattleState`。
...
- 玩法描述：潜影贝默认使用专属均衡控场卡组，不再使用通用 fallback 近战卡组。它会在战斗中主要打出 `潜影弹`、`扰乱潜影弹` 和 `闭壳固守`：前两张分别造成 4 点远程伤害，或造成 3 点远程伤害并施加 1 层缓慢；后者提供 5 点格挡和 1 层相位。潜影贝打出潜影弹类牌时会像原版一样开壳、发射潜影弹，平时保持闭壳。玩家之后获得这些牌时，只保留潜影弹投射和命中表现，不会额外套用潜影贝开壳姿态。
  - 代码实现：`MonsterDeckProfile` 为 `EntityType.SHULKER` 提供固定默认卡组，并让未覆盖奖励池按开战卡组去重后得到这 3 张牌；`MoonSpireCardRegistry` 注册 `builtin_monster_shulker_bullet`、`builtin_monster_disruptive_bullet` 和 `builtin_monster_shell_closure`。`BattleState` 为两张潜影弹牌分配 `BattleVisualEvent.AnimationType.SHULKER_BULLET`、12 tick 准备时间和潜影贝专用发射点；`ClientBattleState`、`BattleWorldOverlay` 与 `ClientEvents` 负责潜影弹投射、命中特效和潜影贝临时开壳恢复。
  - 变更记录：新增潜影贝专属默认卡组、奖励池去重结果和潜影弹世界动画，并修复潜影贝在战前与战斗内不容易被点中的问题。
- 玩法描述：监守者默认使用 10 张专属卡组，并在开战时自带 5 层黑暗。`监守重击` 费用 1，造成 12 点普通攻击伤害；`监守音波` 费用 2，造成 8 点固定伤害；`监守咆哮` 费用 2，对全体敌方施加 2 层黑暗；`幽匿心脏` 费用 2，抽 1 张牌并获得 1 点当前费用。监守者本体打出近战、音波、咆哮牌时会使用对应表现；玩家或其它单位使用时不强求监守者式肢体动作，但仍保留音效，音波牌还会生成音波粒子轨迹。
  - 代码实现：`MoonSpireCardRegistry` 注册 `builtin_monster_warden_maul`、`builtin_monster_warden_sonic_boom`、`builtin_monster_warden_roar` 和 `builtin_monster_sculk_heart`；`MonsterDeckProfile` 为 `EntityType.WARDEN` 提供 4/2/1/3 的默认卡组和奖励池去重结果；`BattleState.applyDefaultInitialEffects(...)` 给监守者追加 5 层 `DARKNESS`。`BattleVisualEvent.AnimationType.WARDEN_MELEE`、`WARDEN_SONIC_BOOM`、`WARDEN_ROAR` 与 `ClientBattleState`、`ClientEvents` 负责对应音效、动画状态和音波粒子。
  - 变更记录：新增监守者默认卡组、默认黑暗初始状态、固定伤害音波、施加黑暗咆哮、幽匿心脏和监守者专属表现。
...
- 玩法描述：怪物意图会预告怪物按当前手牌、费用、生命、格挡和状态计划使用的牌。怪物会按局势给可用牌评分：攻击牌优先击倒玩家，无法击倒时倾向于攻击发光、低生命或低格挡目标；防御牌在低生命、低格挡时收益更高；治疗只在能实际恢复生命时计入收益；增益会倾向给最需要的友方，削弱会倾向给最适合的玩家，已有同类状态较高时会降低收益。苦力怕可用时优先点燃引信；消耗箭牌只有在当前手牌有可消耗箭时才作为有效攻击考虑。怪物一次行动后如果还剩费用和可用手牌，会继续按同一评分规则选择下一张牌；同分时保留手牌靠前者优先。默认怪物卡组、默认属性、专属奖励池和开发者覆盖规则维护入口见 `docs/monster_decks.md`。
  - 代码实现：`BattleState.plannedMonsterCards()` 和 `plannedAutoChoices(...)` 用虚拟手牌、虚拟抽牌堆、虚拟弃牌堆、虚拟费用和 `MonsterAiView` 反复调用 `chooseMonsterCard(...)` 生成意图列表，并模拟已计划牌造成的伤害、格挡、治疗、状态变化、抽牌和获得能量；玩家回合展示意图时 `MonsterTurnPlan.cards(...)` 使用计划中记录的费用顺序解析卡牌 UUID，尚未真实抽到的计划牌使用预览牌显示，敌方回合实际出牌时 `MonsterTurnPlan.pollUsable(...)` 使用当前手牌索引、当前费用和当前目标重新校验。`chooseMonsterCard(...)` 返回 `MonsterCardChoice`，包含卡牌 UUID、预览牌、手牌索引、单体目标、类别、评分、计划抽牌 UUID 和计划回费；`priorityMonsterCard(...)` 会在苦力怕拥有可用 `builtin_monster_light_fuse` 时先返回点燃引信，其它情况再由分类加权随机处理。评分 helper 会统一计算攻击、防御、增益、削弱、手牌处理和消耗箭收益，并把 `CardEffectKind.THORNS` 视为正面状态、把潮蚀、麻痹和饥饿视为负面状态计入评分与意图统计。`tickMonsterTurn()` 消费同一计划出牌并把评分选出的单体目标传入 `queueCard(...)`；多目标和随机目标仍由原有 `targetsForEffect(...)` 结算规则处理。
