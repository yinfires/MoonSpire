...
怪物默认卡组、默认属性、默认初始状态、奖励池、开发者怪物覆盖和回退规则的实现维护说明见 `docs/monster_decks.md`。新增或改动这些规则时，应先阅读并同步更新该文档。
...
如果任一拆分数据文件存在，加载时以拆分文件为当前权威数据源：缺失的同类拆分文件会被视为空列表，而不会从旧的 `developer_data.json` 里把已删除内容复活。怪物默认卡组、属性、奖励池、覆盖清理和回退规则见 `docs/monster_decks.md`。
